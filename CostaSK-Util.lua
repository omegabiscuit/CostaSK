...
CSK_WAIT_FOR_ENABLE_OR_DISABLE = 1
CSK_WAIT_FOR_INSPECT = 2
...
function CSK_CreateEmptyTooltip(rows)
...
function CSK_SendBroadcast(message, isHighPriority)
	SendChatMessage('<CSK> ' .. message, GetBroadcastChannel(isHighPriority))
end	

function CSK_SendWhisper(message, person)
	SendChatMessage('<CSK> ' .. message, 'WHISPER', nil, person)
end

function CSK_SendAlert(message)
	print(GetColoredMessage('<CSK> ', _G.YELLOW_FONT_COLOR_CODE) .. GetColoredMessage(message, _G.GREEN_FONT_COLOR_CODE))
end	

function CSK_SendUserMessage(message)
	print(GetColoredMessage('<CSK> ', _G.YELLOW_FONT_COLOR_CODE) .. GetColoredMessage(message, _G.LIGHTYELLOW_FONT_COLOR_CODE))
end	

function CSK_SendDebugMessage(message)
	if CSK_PREFS[CSK_PREFS_DEBUG] then
		print(GetColoredMessage('<CSK> ', _G.YELLOW_FONT_COLOR_CODE) .. GetColoredMessage(message, _G.GRAY_FONT_COLOR_CODE))
	end		
end	

-- Returns the message that would be whispered when player requests an item
function CSK_GetWhisperMessage(itemLink, message)
	if message == nil then
		message = CSK_PREFS[CSK_PREFS_WHISPER_MESSAGE]
	end
...
function CSK_wait(waitType, delay, func, ...)
	if (type(delay) == 'number' and type(func) == 'function') then
		...
		print('Bad types in CSK_wait')
	end
end

function CSK_PreemptWait(waitType)
